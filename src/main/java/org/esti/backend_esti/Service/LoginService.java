...
public class LoginService {
    ...
    public AuthResponse login(String email, String password) throws UsernameNotFoundException {
        ...
        if (admin != null && passwordEncoder.matches(password, admin.getPassword())) {
            String token = jwtUtil.generateToken(admin.getIdAdmin() ,admin.getEmail(), admin.getRole());

            return buildAuthResponse(admin.getIdAdmin().toString(), admin.getName(), token, admin.getRole());
        }
        ...
        if (teacher != null && passwordEncoder.matches(password, teacher.getPassword())) {
            String token = jwtUtil.generateToken(teacher.getIdTeacher() ,teacher.getEmail(), teacher.getRole());

            return buildAuthResponse(teacher.getIdTeacher().toString(), teacher.getName(), token, teacher.getRole());
        }
